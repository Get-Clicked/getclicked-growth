...
Check Notion: read `.active-client` → `notion-search` for "[Client Name] Workspace" → if found, note section page IDs (NOTION_ENABLED = true). Complete all local files first, then sync to Notion in one pass. Markdown → `notion-update-page`. CSV → `notion-create-pages` (rows). JSON → `notion-update-page` (code block). If Notion unavailable, local files only.
